...
public class OrMatcherTest {
    ...
    void test_constructor_NullInput_ShouldThrowException() {
        assertThrows(NullPointerException.class, () -> new OrMatcher(null));
    }

    @Test
    void test_matches_NullInput_ShouldThrowException() {
        OrMatcher orMatcher = new OrMatcher(Collections.emptyList());
        assertThrows(NullPointerException.class, () -> orMatcher.matches(null));
    }

    @Test
    void test_matches_EmptyList_ShouldReturnFalse() {
        OrMatcher orMatcher = new OrMatcher(Collections.emptyList());
        assertFalse(orMatcher.matches(mock(File.class)));
    }
    ...
    @Test
    void test_matches_SomeTrue_ShouldReturnTrue() {
        ...
        OrMatcher orMatcher = new OrMatcher(Arrays.asList(trueMatcher1, falseMatcher1));
    ...
    void test_matches_NoneTrue_ShouldReturnFalse() {
        ...
        OrMatcher orMatcher = new OrMatcher(Arrays.asList(falseMatcher1, falseMatcher2));
        assertFalse(orMatcher.matches(mock(File.class)));
    }
}
